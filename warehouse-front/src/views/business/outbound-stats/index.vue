<template>
  <div class="page-container">
    <div class="page-header animate-fade-in-up">
      <h1 class="page-header-title">出货统计</h1>
      <p class="page-header-desc">按已确认销售出库单汇总数量与金额，不含红冲单。</p>
    </div>
...
import { loadSalesStats } from '@/api/pharmaStats'
...
const loadData = async () => {
  loading.value = true
  try {
    const res: any = await loadSalesStats({ startDate: range.value[0], endDate: range.value[1] })
    data.value = res.data
  } finally {
    loading.value = false
  }
