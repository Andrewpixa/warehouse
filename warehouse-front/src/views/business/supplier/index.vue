<template>
  <div class="page-container">
    <div class="page-header animate-fade-in-up">
      ...
      <p class="page-header-desc">维护上游供货单位与首营审核状态</p>
    </div>
    <el-card>
      <SearchForm v-model="searchParams" @search="handleSearch" @reset="handleReset">
        <el-form-item label="供应商名称">
          <el-input v-model="searchParams.name" placeholder="供货单位名称" clearable />
        </el-form-item>
      ...
      <CrudTable ref="tableRef" :load-api="loadAllSupplier" :search-params="searchParams">
        <el-table-column prop="id" label="供应商编号" width="110" />
        ...
        <el-table-column label="首营已审" width="90">
          <template #default="{ row }">
            <el-tag :type="row.firstCampOk === 1 ? 'success' : 'info'" size="small">
              {{ row.firstCampOk === 1 ? '是' : '否' }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <CrudDialog ref="dialogRef" :submit-api="handleSubmitApi" :rules="rules" width="560px" @success="tableRef?.reload()">
      <template #default="{ formData }">
        <el-form-item label="供应商编号">
          <el-input :model-value="formData.id ? String(formData.id) : '保存后自动生成（2xxxxx）'" disabled />
        </el-form-item>
        ...
        <el-form-item label="首营已审">
          <el-radio-group v-model="formData.firstCampOk">
...
import { loadAllSupplier, addSupplier, updateSupplier, deleteSupplier } from '@/api/supplier'
...
const handleAdd = () => {
  isEdit.value = false
  dialogRef.value?.open({ status: 1, firstCampOk: 0 }, false)
}
const handleEdit = (row: any) => {
  isEdit.value = true
  dialogRef.value?.open(row, true)
}
const handleSubmitApi = (data: any) => (isEdit.value ? updateSupplier(data) : addSupplier(data))
