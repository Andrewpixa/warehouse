...
const TYPE_CFG: Record<string, any> = {
  ...
  OFFSET: { title: '收款冲账', desc: '按到账金额勾兑发票；金额不一致需挂账，正负对冲需总监确认', noLabel: '冲账单号', addText: '开冲账单', confirmText: '确认冲账', statuses: ['草稿', '已冲账'] },
...
const tagType = (status: string) => {
  ...
  if (['待复检', '待处理', '已登记', '已签名', '已释放'].includes(status)) return 'warning'
  if (status === '拒收') return 'danger'
  return 'info'
}

const canEdit = (status: string) => ['草稿', '待处理', '待分货', '已登记'].includes(status)
